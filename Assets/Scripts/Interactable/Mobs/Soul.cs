...
public class Soul : Interactble, IMob
{
    [SerializeField] private AudioClip _attackSound;
    [SerializeField] private AudioClip _walkSound;
    [SerializeField] private int _steps = 1;
    [SerializeField] private int _stepsDecrease = 1;
    ...
    {
        Debug.Log("Soul attacks player");
        SFXManager.Instance.PlaySoundOnce(_attackSound);
        player.AddSteps(-_stepsDecrease);
    ...
    public void MakeStep()
    {
        ...
        if (_direction)
        {
            for (int i = 0; i < 3; DirIndex++)
            {
                ...
                {
                    Debug.Log($"Soul can move in {dir} direction");
